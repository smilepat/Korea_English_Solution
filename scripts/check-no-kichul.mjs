...
const SNAPSHOT = join(ROOT, "data", "passages", "passages.jsonl")
...
const ALLOWED_SOURCE_PREFIX = ["lexile_textdb_original"]
...
const lines = readFileSync(SNAPSHOT, "utf8").split(/\r?\n/).filter(Boolean)
const violations = []
...
lines.forEach((line, i) => {
  ...
  try {
    ...
    violations.push(`${i + 1}: JSON 파싱 실패`)
    return
  }
  // 1) 금지 구조 필드
  for (const f of FORBIDDEN_FIELDS) {
    if (f in o) violations.push(`${i + 1}: 문항 구조 필드 '${f}' 감지 (기출 유입 의심)`)
  }
  // 2) 라이선스 화이트리스트
  if (!ALLOWED_LICENSE.has(o.license)) {
    violations.push(`${i + 1}: 허용되지 않은 license '${o.license}' (text_id=${o.text_id})`)
  }
  // 3) 소스 화이트리스트
  if (!ALLOWED_SOURCE_PREFIX.some((p) => String(o.source || "").startsWith(p))) {
    violations.push(`${i + 1}: 허용되지 않은 source '${o.source}' (text_id=${o.text_id})`)
  }
})
...
console.log(`✓ 지문 ${lines.length}개 — 기출 문항 구조 없음, license/source 전부 화이트리스트`)
