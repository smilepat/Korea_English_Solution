...
| 테이블 | 행 | 내용 |
|---|---|---|
| kcsdb_standards | 672 | 성취기준(2015 360+2022 312). cefr_alignment 672/672(원본334+상속64+추정274), **cefr_source** 컬럼 |
...
| kcsdb_grammar | 40 | 언어형식(2015). **item_name_ko·category**(Gemini 분류, label_source=llm) |
...
```
> ⚠️ 재시드는 kcsdb_standards를 DELETE+INSERT하므로 `cefr_source`가 초기화됨 → **backfill-cefr.mjs 재실행 필수**. 마찬가지로 grammar 분류·csat맵도 재실행.
...
- kcsdb_grammar 40 = 2015 [별표4] 부분 커버(●학교급 마커 미복원). 학년군 단위 연계가 정직한 한계. LLM 라벨 **전수 검수** 권장(40건).
- CEFR estimated 274(대부분 고교 선택과목=B1 기본값). 과소추정 가능 → 필요 시 과목별 정교화.
